          ...
          <div className="bg-white border-b border-slate-200 shadow-sm">
            <div className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between flex-wrap gap-3">
              <div className="flex items-center gap-3">
                <div className="w-12 h-12 bg-blue-600 rounded-2xl flex items-center justify-center text-white font-black text-xl">{custAcc.profile.firstName.charAt(0)}</div>
                <div>
                  <p className="font-black text-slate-900 text-base">{custAcc.profile.firstName} {custAcc.profile.lastName}</p>
                  <p className="text-xs text-slate-500">{custAcc.profile.city} · {custAcc.profile.phone}</p>
                </div>
              </div>
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <button onClick={()=>{ setCustEditData(custAcc.profile); setCustEditOpen(true); }} className="text-xs font-bold px-3 py-2 bg-white border border-slate-200 text-slate-600 rounded-xl hover:bg-slate-50 transition">{t.editProfile}</button>
              ...
              <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
                ...
                <div className="p-6 space-y-4">
                  <div className="flex gap-4">
                    <img src={selectedBooking.vehicleImg} className="w-28 h-20 rounded-xl object-cover flex-shrink-0" alt=""/>
                    <div>
                      <p className="font-black text-slate-900">{selectedBooking.vehicleName}</p>
                      <p className="text-xs text-slate-500 mt-1">{selectedBooking.shopName} · {selectedBooking.location}</p>
                  ...
                  <div className="bg-slate-50 rounded-xl border border-slate-200 divide-y divide-slate-100">
                    {([
                      [t.rentalPeriod, `${selectedBooking.pickupDate} → ${selectedBooking.returnDate}`],
                      ['Days', `${selectedBooking.days} day${selectedBooking.days>1?'s':''}`],
                      [t.pickupType, selectedBooking.deliveryType==='delivery'?t.delivery:t.selfPickup],
                      [t.vehicleRented, `Rs. ${selectedBooking.pricePerDay.toLocaleString()} /day`],
                      ...(selectedBooking.deliveryType==='delivery' ? [[t.deliveryFee,'Rs. 1,500']] : []),
                      [t.totalPaid, `Rs. ${selectedBooking.total.toLocaleString()}`],
                      [t.status, statusLabel(selectedBooking.status)],
                      [t.bookedOn, new Date(selectedBooking.bookedAt).toLocaleDateString()],
                ...
                {(custAcc.bookings||[])
                  ...
                  <div key={b.id} className="bg-white rounded-2xl border border-slate-200 shadow-sm hover:shadow-md transition overflow-hidden">
                    <div className="flex gap-4 p-4">
                      <img src={b.vehicleImg} className="w-24 h-16 rounded-xl object-cover flex-shrink-0" alt=""/>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-start justify-between gap-2">
                          <div>
                            <p className="font-black text-slate-900 text-sm">{b.vehicleName}</p>
                            <p className="text-xs text-slate-400 mt-0.5">{b.shopName} · {b.location}</p>
                          </div>
                        ...
                        <div className="flex items-center gap-4 mt-1.5 flex-wrap">
                          <span className="text-xs text-slate-500">📅 {b.pickupDate} → {b.returnDate}</span>
                          <span className="text-xs font-black text-slate-900">Rs. {b.total.toLocaleString()}</span>
                          <span className="text-xs text-slate-400">{b.days}d · {b.deliveryType}</span>
                        </div>
                      </div>
                    </div>
                    <div className="border-t border-slate-100 px-4 py-2.5 flex justify-between items-center">
                      <span className="text-[10px] text-slate-400">{t.bookedOn}: {new Date(b.bookedAt).toLocaleDateString()}</span>
        ...
        <div className="bg-slate-100 min-h-[calc(100vh-64px)]">
          <div className="bg-white border-b border-slate-200 shadow-sm">
            <div className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between gap-4 flex-wrap">
              <div className="flex items-center gap-3">
                <div className="w-12 h-12 bg-slate-900 rounded-2xl flex items-center justify-center text-white font-black text-xl">{ownerAcc.profile.shopName.charAt(0).toUpperCase()}</div>
                <div>
                  <p className="font-black text-slate-900 text-base">{ownerAcc.profile.shopName}</p>
                  <p className="text-xs text-slate-500">{ownerAcc.profile.city} · {ownerAcc.profile.phone}</p>
                </div>
              </div>
              <div className="flex items-center gap-2 flex-wrap">
                <div className="hidden sm:flex items-center gap-3 text-xs font-bold bg-slate-50 border border-slate-200 rounded-xl px-4 py-2">
              ...
              <div className="space-y-3">
                {ownerBookings.length===0 ? (
                  ...
                  <div key={b.id} className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                    <div className="flex gap-4 p-4">
                      <img src={b.vehicleImg} className="w-20 h-14 rounded-xl object-cover flex-shrink-0" alt=""/>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-start justify-between gap-2">
                          <div>
                            <p className="font-black text-slate-900 text-sm">{b.vehicleName}</p>
                            <p className="text-xs text-slate-400 mt-0.5">📅 {b.pickupDate} → {b.returnDate} · {b.days}d</p>
                            <p className="text-xs text-slate-400">{b.deliveryType==='delivery'?'🚚 '+t.delivery:'📍 '+t.selfPickup} · <span className="font-black text-slate-900">Rs. {b.total.toLocaleString()}</span></p>
                          </div>
                          <span className={`px-2.5 py-1 rounded-lg text-[10px] font-black uppercase border flex-shrink-0 ${statusColor(b.status)}`}>{statusLabel(b.status)}</span>
                        </div>
                      </div>
