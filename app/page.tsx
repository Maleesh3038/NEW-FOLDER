...
function NearbyVehiclesSection({ allVehicles, onVehicleClick, getPeriodPrice, fmt }: {
  allVehicles: any[];
  onVehicleClick: (v: any) => void;
  getPeriodPrice: (v: any) => { price: number; unit: string; mult: number };
  fmt: (p: number) => string;
}) {
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        {nearby.map(v => (
          <div key={v.id} onClick={() => onVehicleClick(v)}
            ...
            <div className="p-2">
              <p className="text-xs font-black text-slate-900">{fmt(getPeriodPrice(v).price)}</p>
              <p className="text-[10px] text-slate-400">/{getPeriodPrice(v).unit}</p>
            </div>
          </div>
        ))}
      </div>
...
export default function Home() {
  ...
  const [newV, setNewV] = useState({ name: '', type: 'car', transmission: 'Automatic', fuel: 'Petrol', pricePerDay: '' as any, weeklyPrice: '' as any, monthlyPrice: '' as any, kmPerDay: '' as any, extraKmCharge: '' as any, depositAmount: '' as any, description: '', mapLink: '', driverOption: 'self_drive', district: '', deliveryOption: 'both', revenueLicenceExpiry: '', insuranceExpiry: '' });
  ...
  const handleVehicleSubmit = async (e: React.FormEvent) => { e.preventDefault(); if (!newV.name.trim()) { showToast('Vehicle name required!', 'err'); return; } if (!newV.mapLink.trim()) { showToast('📍 Google Maps pickup location is required!', 'err'); return; } const reqPhotos = (newV.type === 'bike' || newV.type === 'tuk') ? 4 : 6;
    if (photos.filter(Boolean).length < reqPhotos) {
      ...
      return;
    } const todayCheck = new Date(); todayCheck.setHours(0, 0, 0, 0); if ((newV as any).revenueLicenceExpiry) { const revExp = new Date((newV as any).revenueLicenceExpiry); if (revExp < todayCheck) { showToast('Revenue Licence is EXPIRED! Renew before listing.', 'err'); return; } } if ((newV as any).insuranceExpiry) { const insExp = new Date((newV as any).insuranceExpiry); if (insExp < todayCheck) { showToast('Insurance is EXPIRED! Renew before listing.', 'err'); return; } } const ownerId = ownerAcc?.id; if (!ownerId) { showToast('Please login again', 'err'); return; } if (editingId) { const { error } = await updateVehicle(editingId, { name: newV.name, type: newV.type as any, transmission: newV.transmission, fuel: newV.fuel, price_per_day: Number(newV.pricePerDay), description: newV.description, map_link: newV.mapLink, driver_option: (newV as any).driverOption || 'self_drive', delivery_option: (newV as any).deliveryOption || 'both',
          delivery_charge: (newV as any).deliveryCharge !== '' ? Number((newV as any).deliveryCharge) || null : null,
          driver_charge: (newV as any).driverCharge !== '' ? Number((newV as any).driverCharge) || null : null,
          driver_charge: (newV as any).driverCharge !== '' ? Number((newV as any).driverCharge) || null : null, revenue_licence_expiry: (newV as any).revenueLicenceExpiry || null, insurance_expiry: (newV as any).insuranceExpiry || null, weekly_price: (newV as any).weeklyPrice !== '' && (newV as any).weeklyPrice !== null ? Number((newV as any).weeklyPrice) : 0, monthly_price: (newV as any).monthlyPrice !== '' && (newV as any).monthlyPrice !== null ? Number((newV as any).monthlyPrice) : 0, km_per_day: (newV as any).kmPerDay !== '' ? Number((newV as any).kmPerDay) || null : null, extra_km_charge: (newV as any).extraKmCharge !== '' ? Number((newV as any).extraKmCharge) || null : null, deposit_amount: (newV as any).depositAmount !== '' ? Number((newV as any).depositAmount) || 0 : 0 }, photos); if (error) { showToast(error, 'err'); return; } showToast('Vehicle updated ✓'); } else { const { id, error } = await addVehicle({
          owner_id: ownerId,
          name: newV.name,
          ...
          description: newV.description || undefined,
          map_link: newV.mapLink || undefined,
          weekly_price: (newV as any).weeklyPrice !== '' ? Number((newV as any).weeklyPrice) || null : null,
          monthly_price: (newV as any).monthlyPrice !== '' ? Number((newV as any).monthlyPrice) || null : null,
          km_per_day: (newV as any).kmPerDay !== '' ? Number((newV as any).kmPerDay) || null : null,
          extra_km_charge: (newV as any).extraKmCharge !== '' ? Number((newV as any).extraKmCharge) || null : null,
          ...
          <div className="bg-white border-b border-slate-200 shadow-sm">
            <div className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between gap-4 flex-wrap">
              ...
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <button onClick={() => { setShowAddForm(true); setEditingId(null); setNewV({ name: '', type: 'car', transmission: 'Automatic', fuel: 'Petrol', pricePerDay: '' as any, weeklyPrice: '' as any, monthlyPrice: '' as any, kmPerDay: '' as any, extraKmCharge: '' as any, depositAmount: '' as any, description: '', mapLink: '', driverOption: 'self_drive', district: '', deliveryOption: 'both', revenueLicenceExpiry: '', insuranceExpiry: '' }); setPhotos([]); }} className="text-xs font-black px-4 py-2 bg-slate-900 hover:bg-slate-800 text-white rounded-xl transition flex items-center gap-1.5 shadow-sm"><span className="text-lg leading-none">+</span> {t.addVehicle}</button>
              </div>
            </div>
                ...
                {(showAddForm || editingId) && (
                  <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                    ...
                    <form onSubmit={handleVehicleSubmit} className="p-6 space-y-4">
                      ...
                      <div><label className="block text-[10px] font-black text-slate-400 uppercase tracking-wider mb-2">🚚 Delivery Option</label><div className="grid grid-cols-3 gap-2">{([['pickup_only', '📍 Self Pickup Only', 'Customer comes to you'], ['delivery_only', '🚚 Delivery Only', 'You deliver'], ['both', '✅ Both Options', 'Customer chooses']] as [string, string, string][]).map(([val, label, note]) => (<button key={val} type="button" onClick={() => setNewV({ ...newV, deliveryOption: val } as any)} className={`py-2.5 px-2 rounded-xl border text-center transition ${(newV as any).deliveryOption === val ? 'bg-slate-900 text-white border-slate-900' : 'bg-slate-50 border-slate-200 text-slate-600 hover:border-slate-400'}`}><p className="text-xs font-black">{label}</p><p className="text-[10px] opacity-60 mt-0.5">{note}</p></button>))}</div>
                      {/* Delivery charge — show when delivery is offered */}
                      {((newV as any).deliveryOption === 'delivery_only' || (newV as any).deliveryOption === 'both') && (
                        <div className="mt-3 bg-amber-50 border border-amber-100 rounded-xl p-3">
                          <label className="block text-[10px] font-black text-amber-700 uppercase tracking-wider mb-2">🚚 Delivery Charge (Rs.) — Fixed fee per delivery</label>
                          <input
                            type="number"
                            min="0"
                            placeholder="e.g. 500"
                            className="w-full bg-white border border-amber-200 rounded-xl px-4 py-2.5 text-sm font-black outline-none focus:border-amber-400 transition placeholder:text-slate-300 placeholder:font-normal"
                            value={(newV as any).deliveryCharge === 0 ? '' : (newV as any).deliveryCharge}
                            onChange={e => setNewV({ ...newV, deliveryCharge: e.target.value === '' ? '' as any : Number(e.target.value) } as any)}
                          />
                          <p className="text-[10px] text-amber-600 mt-1.5">💡 This fixed amount will be added to the booking total when customer selects delivery</p>
                        </div>
                      )}
                      </div>
                      ...
                      {/* Driver charge — show when driver is offered */}
                      {((newV as any).driverOption === 'with_driver' || (newV as any).driverOption === 'both') && (
                        <div className="mt-3 bg-blue-50 border border-blue-100 rounded-xl p-3">
                          <label className="block text-[10px] font-black text-blue-700 uppercase tracking-wider mb-2">🧑‍✈️ Driver Charge (Rs. per day)</label>
                          <input
                            type="number"
                            min="0"
                            placeholder="e.g. 2000"
                            className="w-full bg-white border border-blue-200 rounded-xl px-4 py-2.5 text-sm font-black outline-none focus:border-blue-400 transition placeholder:text-slate-300 placeholder:font-normal"
                            value={(newV as any).driverCharge === 0 ? '' : (newV as any).driverCharge}
                            onChange={e => setNewV({ ...newV, driverCharge: e.target.value === '' ? '' as any : Number(e.target.value) } as any)}
                          />
                          <p className="text-[10px] text-blue-600 mt-1.5">💡 Added per day when customer selects "With Driver" option</p>
                        </div>
                      )}
                      </div>
        ...
        <>
          {view === 'home' && (
            <>
              ...
              <NearbyVehiclesSection allVehicles={allVehicles} onVehicleClick={(v) => { setSelectedVehicle(v); setView('detail'); window.scrollTo({ top: 0, behavior: 'smooth' }); }} getPeriodPrice={getPeriodPrice} fmt={fmt}/>
